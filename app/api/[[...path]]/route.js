...
async function handleRoute(request, { params }) {
  ...
  try {
    ...
    //   Aucun autre champ, aucune autre collection.
    ...
    if (route === '/admin-password-reset' && method === 'POST') {
      ...
      if (!envToken || envToken.length < 32) {
        return handleCORS(NextResponse.json({
          ...
          message: 'L\'endpoint de réinitialisation est désactivé (ADMIN_RESET_TOKEN non configuré).'
        }, { status: 403 }))
      }
      ...
      if (!phone || phone.replace(/[^\d]/g, '').length < 8) {
        return handleCORS(NextResponse.json({
          error: 'INVALID_PHONE',
          message: 'Téléphone requis (format international).'
        }, { status: 400 }))
      }
      ...
      if (!target) {
        return handleCORS(NextResponse.json({
          ...
          message: 'Aucun compte ADMIN trouvé avec ce numéro.'
        }, { status: 404 }))
      }

      const hadPasswordBefore = !!target.passwordHash

      // 2) Hash bcrypt (saltRounds=10, identique au reste de l'app)
      const newHash = await bcrypt.hash(newPassword, 10)
      const now = new Date()

      // 3) UPDATE NON-DESTRUCTIF (sélection par ID unique pour zéro collateral)
      ...
      await db.collection('system_flags').updateOne(
        { id: 'admin_password_reset_used' },
        {
          $set: {
            ...
            usedBy: ip,
            adminId: target.id,
            adminPhone: target.phone
          }
        },
        { upsert: true }
      )

      // 5) Vérification finale (sans afficher le hash)
      ...
      console.log(`[admin-password-reset] ✅ Reset effectué pour admin ${target.id} (${target.phone}) depuis ${ip}`)
      ...
      return handleCORS(NextResponse.json({
        ...
        report: {
          before: {
            ...
            hadPasswordHash: hadPasswordBefore
          },
          after: {
            id: after.id,
            phone: after.phone,
            ...
            hashLength: after.passwordHash?.length || 0,  // attendu : 60 (bcrypt)
        ...
        message: 'Mot de passe réinitialisé avec succès. L\'endpoint est désormais désactivé.'
      }))
    }
