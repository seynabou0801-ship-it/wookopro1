...
export default function HomePage() {
  ...
  const [showQuickWhatsAppForm, setShowQuickWhatsAppForm] = useState(false)
  const [clientName, setClientName] = useState('')
  const [clientPhone, setClientPhone] = useState('') // ⚡ NOUVEAU : Téléphone requis
  ...
  const handleQuickWhatsApp = () => {
    setShowQuickWhatsAppForm(true)
  }

  const handleQuickWhatsAppSubmit = async () => {
    // Validation
    if (!clientName.trim()) {
    ...
    try {
      // ⚡ NOUVEAU : Enregistrer la demande en base de données AVANT d'ouvrir WhatsApp
      const res = await fetch('/api/leads', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
          serviceCategory: 'autre', // Service non spécifié
          city: 'Non spécifié',
          phone: clientPhone,
          description: `Demande via WhatsApp rapide - Client: ${clientName}`,
          source: 'whatsapp_quick_button',
          canal: 'whatsapp'
        })
      })
      ...
      // Message WhatsApp avec référence à la demande
      const message = `Bonjour Wooko,\n\nJe m'appelle ${clientName} et j'ai besoin d'un prestataire.\n\n📞 Mon numéro : ${clientPhone}\n\nMerci !`
      
      // Ouvrir WhatsApp après enregistrement réussi
      openWhatsApp(phone, message)
      
      // Réinitialiser et fermer
      setShowQuickWhatsAppForm(false)
      setClientName('')
      setClientPhone('')
      
      // Message de confirmation
      alert('✅ Votre demande a été enregistrée ! Vous allez être redirigé vers WhatsApp.')
    } catch (error) {
  ...
  return (
    <main className="min-h-screen bg-gradient-to-b from-white via-gray-50 to-white">
      ...
      {/* ⚡ Modal WhatsApp Rapide - NOUVEAU */}
      {showQuickWhatsAppForm && (
        <div className="fixed inset-0 bg-black/50 z-50 flex items-center justify-center p-4">
          <div className="bg-white rounded-2xl shadow-2xl w-full max-w-md p-6 sm:p-8">
            <h3 className="text-2xl font-bold text-gray-900 mb-4">📱 Contact WhatsApp</h3>
            
            <p className="text-gray-600 mb-6">
              Pour enregistrer votre demande et mieux vous servir, merci de remplir ces informations.
            </p>
            ...
            <div className="space-y-4 mb-6">
              <div>
                ...
                <input
                  ...
                  className="w-full px-4 py-3.5 border-2 border-gray-200 rounded-xl focus:outline-none focus:border-[#25D366] focus:ring-4 focus:ring-green-100 transition-all"
                  autoFocus
                  disabled={loading}
                />
              </div>
            ...
            <div className="flex gap-3">
              <button
                ...
                disabled={loading}
                className="flex-1 px-6 py-3.5 bg-gray-200 text-gray-700 rounded-xl font-semibold hover:bg-gray-300 transition-all disabled:opacity-50 disabled:cursor-not-allowed"
              >
                Annuler
              </button>
              <button
                ...
                disabled={loading}
                className="flex-1 px-6 py-3.5 bg-[#25D366] text-white rounded-xl font-bold hover:bg-[#20BA5A] shadow-lg hover:shadow-xl transition-all disabled:opacity-50 disabled:cursor-not-allowed flex items-center justify-center gap-2"
              >
                {loading ? (
                  <>⏳ Enregistrement...</>
                ) : (
